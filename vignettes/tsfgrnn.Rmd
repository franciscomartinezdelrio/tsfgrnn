...
`lags`
    : an integer vector indicating the lagged values used as autorregresive variables (for instance, 1:2 means that lagged values 1 and 2 should be used).
...
As seen in previous examples, when this parameter is omitted in the function call it is automatically selected. The automatic selection is based on maximising a forecast accuracy measure using the rolling origing evaluation.
